...
1. **Создание** — генератор создаёт заказ с координатами и приоритетом
2. **Очередь** — заказ помещается в `PriorityBlockingQueue`
3. **Назначение** — диспетчер находит ближайшее свободное такси
4. **Получение** — такси извлекает заказ из своей очереди
5. **Подача** — такси едет к клиенту (состояние GOING_TO_CLIENT)
6. **Перевозка** — такси везёт клиента (состояние TRANSPORTING)
7. **Завершение** — обновление статистики, возврат в состояние AVAILABLE

### Механизм диспетчеризации

```java
// Поиск оптимального такси
for (Taxi taxi : taxiFleet) {
    if (taxi.isAvailable()) {
        double distance = taxi.location.distanceTo(request.pickup);
...
Система оптимизирована для работы без зависаний:
- Обновление карты каждые 500мс
- Обновление статистики каждую секунду
- Обновление таблиц каждые 1.5 секунды
- Инкрементальное обновление таблиц (только изменённые ячейки)
- Минимальное использование блокировок в EDT потоке

## Разработка

### Добавление нового типа такси

```java
// TaxiType.java
LUXURY(40.0, 2.0, "Люкс")
```
...
```java
// HistoryEvent.java
NEW_EVENT_TYPE("Описание")

// RideHistory.java
public void recordNewEvent(long orderId, ...) {
    addEvent(new HistoryEvent(EventType.NEW_EVENT_TYPE, ...));
}
```

## Техническое задание

Проект выполнен в соответствии с [техническим заданием](promt.md) — Вариант 2: "Система управления беспилотными такси в городе".
